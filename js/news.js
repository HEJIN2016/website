    ...
    .done(function(data){
        ...
        for(var i =0 ;i < index ; i++){
           ...
           // var url = (!newsdata[i].content.split("<img src=\"")[1].split("\"")[0])?("img/news/none.jpg"):(newsdata[i].content.split("<img src=\"")[1].split("\"")[0]);
            console.log(url);
            var html = "<li onclick=\"javascript:window.location.href='newsdetail.php?number="+ i +"'\">" + "<h3>" + newsdata[i].title + "</h3>" +"<div class='col-lg-9 col-md-9 col-sm-9 left-word'><div class='info'><span>" + newsdata[i].author + "</span>" + "<span>"+ newsdata[i].time +"</span>" + "</div><p>" + newsdata[i].firstline + "</p><button class='btn'>Details</button></div><div class='col-lg-3 right-img col-md-3' style='background:url("+ url +");background-size: cover\'></div><div style='clear: both'></div></li>";
